...
install_deps_image_skopeo() {
    ...
    case $OS_RELEASE_ID in
        fedora)
            sudo dnf install -y bats go-md2man docker-distribution httpd-tools
            ;;
        debian)
            sudo apt-get install -y bats go-md2man docker-registry apache2-utils
            ;;
    esac
